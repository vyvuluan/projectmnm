...
function Index() {
    ...
    const handleUpdateStatus = (value, px) => {
        ...
        if (value.id === 4 && px.status !== 4 && px.status !== 5) {
            swal({
                ...
                if (value === 'yes') {
                    axios.put(`/api/kho/setstatusPX/${px.id}`, data).then(res => {
                        if (res.data.status === 200) {
                            setSubmitting(true);
                        } else if (res.data.status === 400) {
    ...
    const SortStt = (e) => {
        ...
        if (key !== '') {
            axios.get(`/api/kho/locpx_kho?key=${key}&value=${e}`).then(res => {
                if (res.data.status === 200) {
                    setPxlist(res.data.data.data);
                    setTotalPage(res.data.data.total);
                    setPerPage(res.data.data.per_page);
    ...
    return (
        <>
            <B.Container fluid>
                ...
                {/* In phieu xuat va phieu xuat kho */}
                    ...
                    <B.Tabs activeKey={pxtab} onSelect={(k) => setpxtab(k)}>
                        <B.Tab eventKey={1} title="Hóa đơn">
                            <div ref={componentRef}>
                                <Bill
                                    billCode={viewPx?.id}
                                    hotenKH={viewPx?.tenKH}
                                    sdt={viewPx?.sdt}
                                    diaChi={viewPx?.diaChi}
                                    pttt={viewPx?.pt_ThanhToan}
                                    data={viewPx}
                                    discount={viewPx?.discount}
                                    tongTien={viewPx?.tongTien}
                                />
                            </div>
                        </B.Tab>
